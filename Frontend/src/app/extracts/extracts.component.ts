...
export class ExtractsComponent implements OnInit {
  ...
  constructor(private extractsService: ExtractsService, private categoryService: CategoryService, private route: ActivatedRoute, private router: Router) { }
  ...
  onEdit(extract) {
    this.extractsService.currentExtract = extract;
    if (extract.categoria.receita_ou_despesa == 0) {
      //despesa
      this.router.navigate(['entry-expense', extract.id]);
    } else {
      this.router.navigate(['entry-revenue', extract.id]);
    }
  }

  onDelete(extract) {
    //não está implementado no backend ainda
  }
}
